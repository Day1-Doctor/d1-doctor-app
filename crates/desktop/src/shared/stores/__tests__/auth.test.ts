...
describe('auth store', () => {
  ...
  describe('checkAuth', () => {
    it('sets authenticated when token exists', async () => {
      // Return a fake JWT with email in payload
  ...
  describe('login', () => {
    it('stores token and sets authenticated on success', async () => {
      mockInvoke
        .mockResolvedValueOnce('jwt-token-here') // auth_login
        .mockResolvedValueOnce(undefined)          // store_auth_token
      ...
      expect(mockInvoke).toHaveBeenCalledWith('auth_login', { email: 'test@example.com', password: 'password' })
      expect(mockInvoke).toHaveBeenCalledWith('store_auth_token', { token: 'jwt-token-here' })
    })
    ...
    it('sets loading true during login', async () => {
      ...
      mockInvoke.mockReturnValueOnce(new Promise<string>(r => { resolveLogin = r }))

      const store = useAuthStore()
      const promise = store.login('test@example.com', 'pass')

      expect(store.loading).toBe(true)

      resolveLogin!('token')
      mockInvoke.mockResolvedValueOnce(undefined) // store_auth_token
      await promise
  ...
  describe('loginWithGoogle', () => {
    it('stores token and sets authenticated on success', async () => {
      ...
      const fakeToken = 'header.' + fakePayload + '.signature'
      mockInvoke
        .mockResolvedValueOnce(fakeToken) // auth_google_sso
        .mockResolvedValueOnce(undefined)  // store_auth_token
  ...
  describe('logout', () => {
    it('clears token and sets unauthenticated', async () => {
      mockInvoke.mockResolvedValueOnce(undefined) // clear_auth_token
